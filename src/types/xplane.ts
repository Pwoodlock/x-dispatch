...
export interface PlaneState extends Coordinates {
   ...
   * Sourced from X-Plane dataref `sim/flightmodel/failures/onground_any`
   * (number, 1 = on ground, 0 = airborne). Used by the TAWS terrain layer
   * to suppress the "all-red when parked" UX bug.
   */
  onGround: boolean;
}
